...
/**
 * A stripped down version of https://github.com/cloudbees/diff4j/blob/master/src/main/java/com/cloudbees/diff/ContextualPatch.java
 * With support for access based fuzzing and some helpers
 */
public final class ContextualPatch {
    ...
    private final List<SinglePatch> patches = new ArrayList<SinglePatch>();
    ...
    private boolean patchLineRead = true;
    private int lastPatchedLine;    // the last line that was successfuly patched

    public static SingleFileContext context(String data) {
        return new SingleFileContext(data);
    }

    public static SingleFileContext context(InputStream data) throws IOException {
        return new SingleFileContext(data);
    }

    public static ContextualPatch create(String patchString) {
        try {
            return new ContextualPatch(new BufferedReader(new StringReader(patchString)));
        } catch (Throwable e) {
            return sneak(e);
        }
    }
    public static ContextualPatch create(InputStream input) throws IOException {
        try {
            return new ContextualPatch(new InputStreamReader(input, StandardCharsets.UTF_8));
        } catch (PatchException e) {
            return sneak(e);
        }
    }

    private ContextualPatch(Reader reader) throws IOException, PatchException {
        patchReader = new BufferedReader(reader);
        for (SinglePatch patch; (patch = getNextPatch()) != null; )
            patches.add(patch);
    }
    ...
    public PatchReport patchSingle(boolean dryRun, IContextProvider context) {
        if (this.patches.size() != 1)
            throw new IllegalStateException("Unexpected multi-patch file");
        var patch = this.patches.getFirst();
        try {
            return applyPatch(patch, dryRun, context);
        } catch (Exception e) {
            return new PatchReport(patch.targetPath, patch.binary, PatchStatus.Failure, e, new ArrayList<HunkReport>());
        }
    }
     ...
     */
    public List<PatchReport> patch(boolean dryRun, IContextProvider context) {
        List<PatchReport> report = new ArrayList<PatchReport>();
        for (SinglePatch patch : patches) {
            try {
                report.add(applyPatch(patch, dryRun, context));
    ...
    private PatchReport applyPatch(SinglePatch patch, boolean dryRun, IContextProvider context) {
        lastPatchedLine = 1;
        List<HunkReport> ret = new ArrayList<HunkReport>();

        List<String> target = context.getData(patch.targetPath);
        ...
        if (target != null && !patch.binary) {
            try {
                ...
                // something went wrong, try doing normal patch
            }
        } else if (target == null) {
        ...
        if (!dryRun)
            context.setData(patch.targetPath, target);
    ...
    private HunkReport applyHunk(List<String> target, Hunk hunk, int idx, boolean dryRun, int fuzz, int hunkID) throws PatchException {
        ...
        for (String hunkLine : hunk.lines) {
            ...
            if (!isAddition) {
                ...
                var clean = target.get(idx);
                boolean match = similar(clean, hunkLine.substring(1), hunkLine.charAt(0));
    ...
    private SinglePatch getNextPatch() throws IOException, PatchException {
        ...
        for (; ; ) {
            ...
            if (line.startsWith("Index:")) {
                ...
                throw new IllegalArgumentException("Invalid patch format, Context Format not supported");
            } else if (isNormalDiffRange(line)) {
                throw new IllegalArgumentException("Invalid patch format, Normal Format not supported");
            }
        }
        return patch;
    }
    ...
    private void readBinaryPatchContent(SinglePatch patch) throws PatchException, IOException {
        ...
        Hunk hunk = new Hunk(0, 0, 0, 0, new ArrayList<>());
        for (; ; ) {
            String line = readPatchLine();
    ...
    private Hunk parseRange(String range) throws PatchException {
        Matcher m = unifiedRangePattern.matcher(range);
        if (!m.matches())
            throw new PatchException("Invalid unified diff range: " + range);

        int baseStart = Integer.parseInt(m.group(1));
        int baseCount = m.group(2) != null ? Integer.parseInt(m.group(2).substring(1)) : 1;
        int modifiedStart = Integer.parseInt(m.group(3));
        int modifiedCount = m.group(4) != null ? Integer.parseInt(m.group(4).substring(1)) : 1;
        return new Hunk(baseStart, baseCount, modifiedStart, modifiedCount, new ArrayList<>());
    }
    ...
    public static class SingleFileContext implements ContextualPatch.IContextProvider {
        private List<String> data;

        private SingleFileContext(String base) {
            data = NewLineDetector.readLines(base);
        }

        private SingleFileContext(InputStream base) throws IOException {
            data = NewLineDetector.readLines(base);
        }
